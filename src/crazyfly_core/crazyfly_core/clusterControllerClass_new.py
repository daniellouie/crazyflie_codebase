...
from .cluster_new import Cluster_new 
...
class ClusterController_new(Node):
    def __init__(self):
        ...
        self.waypoints = [
            ...
            [1.5, 0.0, 1.0, np.pi, 0, np.pi, np.pi, 1],
            [1.5, 0.0, 1.0, -np.pi/2, 0, -np.pi/2, -np.pi/2, 1],
            [1.5, 0.0, 1.0, 0 , 0, 0, 0, 1],  #NOTE: THIS MIGHT WANT 2PI
            [1.5, 0.0, 1.0, np.pi/2, 0, np.pi/2, np.pi/2, 1]
        ]
        ...
        self.waypoint_hold_time = 4.0 #in seconds
        self.waypoint_start_time = None
        self.waypoint_tolerance = 0.4 # in meters
    ...
    def publish_commands(self):
        ...
        # des_cf2_msg.po'fake_thrust_pub'se.position.z = safe_cf2_cmd[2]
    ...
    def publish_cluster_msgs(self):
        ...
        cur_cluster_msg.data = np.array(self.cluster.C_cur).tolist()
        self.pub_cur_cluster.publish(cur_cluster_msg)

        # Publish the desired cluster state
        des_cluster_msg = Float32MultiArray()
        des_cluster_msg.data = np.array(self.cluster.C_des).tolist()
        ...
        cmd_cluster_msg.data = np.array(self.cluster.forwardKinematics(self.cluster.R_cmd_ours)).tolist()
    ...
    def check_waypoint_reached(self):
        cur_position = self.cluster.C_cur[:3]
        des_position = self.cluster.C_des[:3]
    ...
    def record_data(self):
        ...
        if (current_time - self.last_capture_time).total_seconds() >= self.frame_capture_interval:
            ...
            cur_cluster_our_frame = self.cluster.C_cur
            des_cluster_our_frame = self.cluster.C_des
...
def main(args=None):
    ...
    controller = ClusterController_new()
    controller.run()
