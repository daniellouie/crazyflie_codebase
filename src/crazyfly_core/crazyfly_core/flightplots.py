...
def FILE_INITIATION(i=0):
    ...
    CLUSTER_DATA_DIR      = WORKSPACE / "cluster_data"
    ...
    CLUSTER_PATTERN     = "cluster_data_*.csv"                             # matches all cluster logs
    CLUSTER_DOT_PATTERN = "cluster_dot_*.csv"
    CF1_TUNING_PATTERN = "cf1_tuning_*.csv"
    CF2_TUNING_PATTERN = "cf2_tuning_*.csv"
    CF1_CMD_PATTERN = "cf1_all_values_*.csv"

    cluster_files   = sorted(CLUSTER_DATA_DIR.glob(CLUSTER_PATTERN))                #finds the file through glob and sorted
    I_Joc_files = sorted(INV_DATA_DIR.glob(CLUSTER_DOT_PATTERN))
    cf1_tuning_files = sorted(CF1_TUNING.glob(CF1_TUNING_PATTERN))
    cf2_tuning_files = sorted(CF2_TUNING.glob(CF2_TUNING_PATTERN))
    
    cf2_files = sorted(CF2_RESULTS.glob(CF2_TUNING_PATTERN))
    cf1_files = sorted(CF1_RESULTS.glob(CF1_TUNING_PATTERN))
    cf1_cmd_files = sorted(CF1_COMMANDS.glob(CF1_CMD_PATTERN))

    # ERROR STATEMENT
    if not cluster_files:
        raise FileNotFoundError(f"No files matching {CLUSTER_PATTERN} in {CLUSTER_DATA_DIR}")
    if not I_Joc_files:
        raise FileNotFoundError(f"No file matching {CLUSTER_DOT_PATTERN} in {INV_DATA_DIR}")
    if not cf2_files:
        raise FileNotFoundError(f"No file matching {CF2_TUNING_PATTERN} in {CF2_TUNING}")
    if not cf1_files:
        raise FileNotFoundError(f"No file matching {CF1_TUNING_PATTERN} in {CF1_TUNING}")
    if not cf1_cmd_files:
        raise FileNotFoundError(f"No file matching {CF1_CMD_PATTERN} in {CF1_COMMANDS}")

    cluster_path   = cluster_files[-3]            # newest because the timestamp sorts lexicographically
    ...
    return cluster_path, I_joc_path, cf2_path, cf1_path, cf1_cmd_path

def load_position_data_xyz(cluster_path):
...
def load_position_data(cluster_path):
    df = pd.read_csv(cluster_path)
    df['Timestamp'] = pd.to_datetime(df['Timestamp'])
    df = df.dropna(subset=['Timestamp', 'Cur_CF1_Y', 'Des_CF1_Y'])

    t = (df["Timestamp"] - pd.Timestamp("1970-01-01")) / pd.Timedelta('1s')
    t = t.to_numpy()
    t -= t[0]
    cur_cf1_positions = df[['Cur_CF1_X', 'Cur_CF1_Z', 'Cur_CF1_Y']].to_numpy()
    cur_cf2_positions = df[['Cur_CF2_X', 'Cur_CF2_Z', 'Cur_CF2_Y']].to_numpy()
    cur_cluster_positions = df[['Cur_Cluster_X', 'Cur_Cluster_Z', 'Cur_Cluster_Y', 'Cur_Alpha',
                                'Cur_Beta',      'Cur_Phi1',      'Cur_Phi2',      'Cur_P'     ]].to_numpy()
    des_cluster_positions = df[['Des_Cluster_X', 'Des_Cluster_Z', 'Des_Cluster_Y', 'Des_Alpha',
                                'Des_Beta',      'Des_Phi1',      'Des_Phi2',      'Des_P'     ]].to_numpy()
    des_cf1_positions = df[['Des_CF1_X', 'Des_CF1_Z', 'Des_CF1_Y']].to_numpy()
    des_cf2_positions = df[['Des_CF2_X', 'Des_CF2_Z', 'Des_CF2_Y']].to_numpy()

    return t, cur_cf1_positions, cur_cf2_positions, cur_cluster_positions, des_cluster_positions, des_cf1_positions, des_cf2_positions
...
#-----------------------------------------------
"""
def plot_cluster_3d(cur_cf1_positions, cur_cf2_positions, cur_cluster_positions, des_cluster_positions, des_cf1_positions, des_cf2_positions):
...
def plot_single_drone_3d():

    path = FILE_INITIATION()
...
def read_from_csv(path):
    ...
    invalid = data[["x", "y", "z"]].isna().any(axis=1)
    data.drop(data[invalid].index, inplace=True)
    
    t = t[~invalid.to_numpy()]

    return t, data

def plot_single_drone():
    # --- locate data (file or directory) ---
    path = select_file(WORKSPACE)

    t, data = read_from_csv(path)
    y = data["y"].to_numpy()
    x = data["x"].to_numpy()
    z = data["z"].to_numpy()
    ...
    # Z (top)
    axes[0].plot(t, z, "r-", label="Z Position", marker=".", markersize=5)
    axes[0].set_xlabel("Time(s)")
    axes[0].set_ylabel("Z Position (m)")
    axes[0].set_title("Z Position Over Time")
    axes[0].legend()

    # X (middle)
    axes[1].plot(t, x, "r-", label="X Position", marker=".", markersize=5)
    ...
    # Y (bottom)
    axes[2].plot(t, y, "r-", label="Y Position", marker=".", markersize=5)
    axes[2].set_xlabel("Time(s)")
    axes[2].set_ylabel("Y Position (m)")
    axes[2].set_title("Y Position Over Time")
    axes[2].legend()

    plt.show()

def plot_single_drone_w_ctrl_effort():
    """
...
def plot_cluster():
    cluster_file = select_file(initialdir=WORKSPACE / "cluster_data")
    t, R1s, R2s, Cs, Cdess, Rdes1s, Rdes2s = load_position_data(cluster_file)
    # TODO plot Rs

    # TODO this won't deal with angle wrap properly
    Cerrs = Cdess - Cs

    C_rows_per_fig = 3
    C_cols_per_fig = 3

    C_titles = ["Cluster x", "Cluster z", "Cluster y", "alpha", "beta", "phi_1", "phi_2", "p"]
    R_titles = ["x1", "y1", "z1", "psi1","x2", "y2", "z2", "psi2"]
    
    # ------------- plotting -------------------
    C_figs_reqd = int(np.ceil(8 / (C_rows_per_fig * C_cols_per_fig)))
    C_figs = []
    C_axs = []
    for _ in range(C_figs_reqd):
        fig, axes = plt.subplots(C_rows_per_fig, C_cols_per_fig, sharex=True)
        C_figs.append(fig)
        C_axs.append(axes)
    
    for i in range(C_figs_reqd):
        for j in range(C_cols_per_fig):
            for k in range(C_rows_per_fig):
                ind = i * C_rows_per_fig * C_cols_per_fig + j * C_rows_per_fig + k
                if ind < 8:
                    scale_factor = 1
                    if 3 < ind < 7:
                        scale_factor = 180 / np.pi
                    # else:
                    C_axs[i][k, j].plot(t, scale_factor * Cs[:, ind], label=" ")
                    C_axs[i][k, j].plot(t, scale_factor * Cdess[:, ind], label=" ")
                    C_axs[i][k, j].plot(t, scale_factor * Cerrs[:, ind], label=" ")
                    # C_axs[i][k, j].plot(t, scale_factor * Ccmds[:, ind], label=" ")
                    # C_axs[i][k, j].plot(time_data, Cdot_dess[:, ind], label="Cdot_des")
                    C_axs[i][k, j].set_title(C_titles[ind])
                    if ind < 3:
                        C_axs[i][k, j].set(ylabel="meters")
                    elif ind < 7:
                        C_axs[i][k, j].set(ylabel="degrees")
                    if k == C_rows_per_fig - 1:
                        C_axs[i][k, j].set(ylabel="meters")
                        C_axs[i][k, j].set(xlabel="seconds")
        
        handles, labels = C_axs[i][0, 0].get_legend_handles_labels()
        C_figs[i].legend(handles, ["Actual", "Desired", "Error", "Ccmds", "Cdot_des"], loc='center left')
        # these settings work well for full screen viewing on my laptop with 6x2 row x col
        C_figs[i].subplots_adjust(left=0.1, bottom=0.06, right=0.96, top=0.96, wspace=0.12, hspace=0.28)  
...
def animate_cluster_3d():
    cluster_file = select_file(initialdir=WORKSPACE / "cluster_data")
    _, cur_cf1_positions, cur_cf2_positions, cur_cluster_positions, \
        des_cluster_positions, des_cf1_positions, des_cf2_positions = load_position_data_xyz(cluster_file)
...
# ------------------------------------- CLUSTER ACCURACY ---------------------------------------------
# TODO should probably be moved to another file since this isn't plotting
...
def main(plot_type):
    print(plot_type.type)
    match plot_type.type.lower():
        case "single":
            plot_single_drone()
        case "cluster":
            plot_cluster()
        case "singleweffort":
            plot_single_drone_w_ctrl_effort()
        case "single3d":
            plot_single_drone_3d()
        case "cluster3d":
            plot_cluster_3d()
        case "animatecluster3d":
            animate_cluster_3d()
        case "ijac":
            plot_I_jac()
        case _:
            print("Provided plot type did not match any plot types.\nValues are:\nsingle, cluster, singleweffort, single3d, cluster3d, animatecluster3d, ijac")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="Flight Plotting",
        description="Plot flight data in a csv file you select. Argument determine the kind of plot."
    )
    parser.add_argument('-t', '--type', default='cluster')
